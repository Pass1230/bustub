...
Page *BufferPoolManager::FetchPageImpl(page_id_t page_id) {
  ...
  // If the buffer pool is full and all pages are pinned(no victim)
  if (frame_id == -1) { return nullptr; }

  page_id_t victim_page_id = pages_[frame_id].page_id_;

  // Check whether the victim page is dirty
  ...
  UpdateMetadata(frame_id, page_id);
  pages_[frame_id].pin_count_++;
...
Page *BufferPoolManager::NewPageImpl(page_id_t *page_id) {
  ...
  *page_id = new_page_id;
  ...
  // Find a replacement page from either the free list or the replacer.
  if (!free_list_.empty()) {
    frame_id = free_list_.front();
    free_list_.pop_front();
  } else {
  ...
  // Update the metadata and add the new page to the buffer pool
  UpdateMetadata(frame_id, new_page_id);
  pages_[frame_id].pin_count_++;
  replacer_->Pin(frame_id);
...
void BufferPoolManager::UpdateMetadata(frame_id_t frame_id, page_id_t page_id) {
  pages_[frame_id].ResetMemory();
